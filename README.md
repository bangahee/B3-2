...
현재 구현은 교육 목적의 메모리 기반 Mini Git에 적합하도록 구성하였습니다.  
커밋 수가 증가하거나 일부 요구사항이 변경될 경우에는 다음과 같은 개선을 고려할 수 있습니다.

### 커밋 수가 증가할 때 예상되는 병목

#### LOG

`LOG`는 모든 커밋과 부모 연결을 확인합니다.

```text
O(V + E)
```

따라서 커밋 수가 10배 증가하면 탐색뿐 아니라 터미널에 출력해야 하는 데이터도 크게 증가합니다.
...
현재 BFS 큐에는 현재 위치까지의 전체 경로를 저장합니다.

```python
queue.append(current_path + [neighbor_hash])
```

이 방식은 이해하기 쉽지만, 그래프가 커질수록 경로 리스트를 반복해서 복사하므로 메모리 사용량이 증가할 수 있습니다.

개선 방법은 큐에는 commit hash만 저장하고, 별도의 `previous` 딕셔너리로 이전 커밋을 기록하는 것입니다.
...
목적지에 도달한 후 `previous`를 역으로 따라가며 최종 경로를 만들 수 있습니다.

#### 브랜치 라벨

현재 `_branch_labels()`는 하나의 커밋에 표시할 브랜치를 찾기 위해 전체 브랜치를 확인합니다.

커밋 수를 `n`, 브랜치 수를 `b`라고 하면 전체 LOG 출력 과정에서 다음 비용이 발생할 수 있습니다.

```text
O(n × b)
```

브랜치 수가 많아질 경우 다음과 같은 역방향 구조를 추가할 수 있습니다.

```text
commit hash -> branch name 목록
```

이를 통해 특정 커밋을 가리키는 브랜치를 더 빠르게 찾을 수 있습니다.

---

### PATH를 부모 방향으로만 탐색하도록 변경할 경우

현재 PATH는 부모-자식 연결을 무방향으로 취급합니다.
...
현재 구현에서는 다음 두 경로를 모두 탐색할 수 있습니다.
...
부모 방향만 허용하면 새 커밋에서 과거 커밋으로만 이동할 수 있습니다.
...
이 경우 `_get_neighbors()`에서 자식 목록을 제외하고 부모만 반환하도록 변경하면 됩니다.
...
BFS 자체는 계속 사용할 수 있지만, `children` 인접 리스트는 PATH 탐색에 필요하지 않게 됩니다.

---

### 작성자 정렬과 부모 우선 조건을 동시에 만족해야 할 경우

현재 `LOG --sort-by=author`는 모든 커밋을 작성자 이름 기준으로 Merge Sort합니다.

따라서 작성자 순서는 만족하지만 부모가 반드시 자식보다 먼저 나온다는 조건까지 동시에 보장하지는 않습니다.

두 조건을 모두 만족해야 한다면 다음 전략을 사용할 수 있습니다.
...
자식의 미처리 부모 수 감소
    ↓
모든 부모가 처리된 자식을 후보에 추가
```

이 방식에서는 부모가 모두 출력된 커밋만 정렬 후보가 되므로 부모-자식 순서를 유지할 수 있습니다.

표준 정렬 API를 사용할 수 없으므로 후보 중 최소 작성자를 직접 선형 탐색하거나 직접 구현한 Heap을 사용할 수 있습니다.

---

### 카운터 기반 hash와 난수 기반 hash 비교

현재 구현은 다음 값을 SHA-1 입력으로 사용합니다.
...
- 실제 Git의 내용 기반 hash와 차이가 있음
- 카운터 값이 hash 결과에 영향을 줌
...
- 실행할 때마다 결과가 달라짐
- 테스트 재현성이 낮음
- 같은 오류 상황을 다시 만들기 어려움
- 예상 hash를 이용한 테스트가 어려움
- 난수 방식에서도 충돌 검사는 필요함

현재 방식은 카운터와 기존 커밋 딕셔너리 확인을 함께 사용하므로 세션 내 hash 중복을 방지하면서 테스트와 디버깅 흐름을 비교적 명확하게 유지합니다.

---

### docstring과 주석 작성 기준

주요 클래스와 함수의 docstring에는 다음 내용을 중심으로 작성하였습니다.
...
주석은 단순히 코드 내용을 반복하기보다 다음과 같이 코드만으로 파악하기 어려운 이유를 설명할 때 작성하였습니다.

```python
# 같은 거리의 경로도 허용하여
# 여러 최단 경로 중 사전순 최소 경로를 비교할 수 있도록 한다.
```

다음과 같이 코드 자체로 의미가 분명한 부분에는 불필요한 주석을 최소화하였습니다.
